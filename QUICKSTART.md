...
This SDK is composed by 4 special parts:
...
## Elements ##
The Elements is the object tree class that represents the structure of an Instant Article. This object tree structure ensure that no invalid Instant Article HTML markup will be generated.
...
Using the **$article** created on the above code snippet is possible to render the correct InstantArticle markup by simply:
...
#### Output of an object InstantArticle rendered ####
```xml
<!doctype html>
<html>
<head>
...
## Transformer ##
The transformation process is the part where a user custom made markup needs to be interpreted to fill in the [InstantArticle](https://github.com/facebook/facebook-instant-articles-sdk-php/blob/master/src/Facebook/Instantarticles/Elements/InstantArticle.php) object structure.

**Given an simple example HTML markup:**
```html
<html>
    <head>
...
```
This HTML can be the one generated by any CMS like Wordpress, Drupal and others like in-house developed CMS's.

### Transformer Configuration ###

The configuration rules are applied **bottom-up**, so all new or custom rules should be added by the end of the file.
...
So this transformer will run through all elements, and for each element checking all rules.
...
#### Matching selector ####
The **selector** field will be used only by rules that extends [ConfigurationSelectorRule](https://github.com/facebook/facebook-instant-articles-sdk-php/blob/master/Qsrc/Facebook/InstantArticles/Transformer/Rules/ConfigurationSelectorRule.php).
...
Each custom rule implemented should comply with full contract of Rule abstract class.
...
To transform your markup into a InstantArticle markup consists in a few steps:
- Creating an InstantArticle instance
- Create Transformer and load it with rules (from file or programmatically)
